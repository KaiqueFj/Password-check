...
# hash is a function that generates a value to each input
...
def request_api_data(query_char):
    ...
    if res.status_code != 200:
        raise RuntimeError(
            f'error_fetching: {res.status_code}, check the api and try again')
    return res
...
def pwned_api_check(password):
    # check password fi it exists in Api response
